    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        // 2. GET avec ID = détail de l'entrée X
            ...
            if (is_numeric($args['id'])) {
                ...
                $sql = "SELECT firstname, lastname, sex, email, role_id
                        FROM users 
        ...
        // 4. PUT/PATCH avec ID = Modification de l'entée X
            ...
            if (is_numeric($args['id'])) {
                ...
                if (
                    ...
                    && isset($_PUT['password']) && $_PUT['password'] != '' &&
                    isset($_PUT['email']) && $_PUT['email'] != ''
                    && isset($_PUT['role_id']) && $_PUT['role_id'] != ''
                ) {
                    ...
                    if(!$uppercase || !$lowercase || !$number || strlen($_REQUEST['password']) < 8) {*/
                        if (!$this->passwordCheck($_PUT['password'])) {
                            ...
                            $stmnt->bindValue(":role_id", $_PUT['role_id'], PDO::PARAM_STR);
                    ...
                    $data['status'] = 'error';
                    $data['code'] = 'paramMissing';
                    $data['message'] = 'Désolé, tous les paramètres sont obligatoire: firstname, lastname, email, sex, password, role_id';
                }
            } else {
                $data['status'] = 'error';
                $data['code'] = 'idMustBeNumeric';
